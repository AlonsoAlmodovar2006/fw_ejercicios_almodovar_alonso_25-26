...
class ViewService {
    // Responsabilidades
    async pintarSelect(select: HTMLSelectElement | null, array: string[]) {
        array.forEach((item) => {
    ...
    async pintarCards(arrayRecetas: MyMeal[], divContenedor: HTMLDivElement | null): Promise<void> {
        if (divContenedor) {
    ...
    pintarCard(receta: MyMeal): HTMLDivElement {
    ...
    ocultarModal(modal: HTMLDivElement) {
        if (modal) {
